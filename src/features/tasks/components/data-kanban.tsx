...
export const DataKanban = ({ data, onChange }: DataKanbanProps) => {
  const [tasks, setTasks] = useState<TasksState>(() => {
    ...
    Object.keys(initalTasks).forEach((stauts) => {
      initalTasks[stauts as TaskStatus].sort((a, b) => a.position - b.position);
    });
  ...
  useEffect(() => {
    ...
    Object.keys(newTasks).forEach((stauts) => {
      newTasks[stauts as TaskStatus].sort((a, b) => a.position - b.position);
    });
    setTasks(newTasks);
  }, [data]);
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      setTasks((prevTasks) => {
        ...
        if (!movedTask) {
          console.error("No task found on the source index");
          return prevTasks;
        }
