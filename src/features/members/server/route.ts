...
.get('/',sessionMiddleWare,
    ...
    const remainingAdmins = allMembersInWorkspace.documents.filter(
        (m) => m.role === MemberRole.ADMIN && m.$id !== memberToDelete.$id
    );

    if (member.$id === memberToDelete.$id && remainingAdmins.length === 0) {
...
,  async(c)=>{
    ...
    const allMembersInWokspace = await databases.listDocuments(DATABASE_ID,
        MEMBERS_ID,
...
if(allMembersInWokspace.total ===1){
